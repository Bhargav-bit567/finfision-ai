          ...
          recentTrades.map((trade, i) => (
            ...
            >
              <div className="trade-main">
                ...
                <span className="asset">₹{trade.entry.toLocaleString("en-IN")}</span>
              </div>
              <div className="trade-details">
                <span className="amount">₹{trade.amount.toLocaleString("en-IN")}</span>
                <span className={`status-badge ${trade.status}`}>
                  {trade.status === 'open' ? `${trade.remaining}s` : (trade.payout >= 0 ? `+₹${trade.payout.toLocaleString("en-IN")}` : `-₹${Math.abs(trade.payout).toLocaleString("en-IN")}`)}
                </span>
              </div>
