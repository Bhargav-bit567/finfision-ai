...
const EmotionPanel = () => {
  const { fearScore, fearReason, fearAnalysis, patterns, disciplineScore, scoreTrend } = useBehaviorStore();
  const confidence = patterns?.confidenceScore || 68;
  const drivers = fearAnalysis?.drivers || [];
  const stats = fearAnalysis?.stats || {};
  ...
  return (
    <div className="emotion-panel-wrapper">
      <div className="metric-card glass-card dashboard-card-lift">
        ...
        <div className="portfolio-fear-analysis">
          <h4>Fear analysis result</h4>
          <p>{fearAnalysis?.result || "Trade in the simulator to build a live fear profile."}</p>
          <div className="fear-stat-grid">
            <span>Avg decision <strong>{stats.averageDecisionSeconds ?? 5}s</strong></span>
            <span>Loss streak <strong>{stats.lossStreak ?? 0}</strong></span>
            <span>Open trades <strong>{stats.openTrades ?? 0}</strong></span>
            <span>Drawdown <strong>{stats.drawdownPercent ?? 0}%</strong></span>
          </div>
          {drivers.length > 0 ? (
            <div className="fear-driver-stack">
              {drivers.map((driver) => (
                <div key={driver.key} className="fear-driver-row">
            ...
            <div className="fear-driver-empty">No pressure driver detected yet.</div>
          )}
        </div>
      </div>
